...
DATA_FILE = DATA_DIR / "nexus_topology_state.json"
SECRET_FILE = DATA_DIR / "nexus_crypto_seed.key"
...
async def sync_cluster_topology(skip_structure=False, force_pull=False):
    ...
    raw = await fetch_edge_checkpoint("Nexus_Core_State")
    if not raw: return
...
def obf_secret(text: str) -> str:
    ...
    key = os.environ.get("MASTER_KEY", AUTH.get("password_hash", "NexusEngine"))
...
async def manual_edge_upload(_=Depends(require_auth)):
    ...
    if EDGE_KV_CONFIG.get("worker_url") and EDGE_KV_CONFIG.get("token"):
        cf_success = await commit_edge_checkpoint("Nexus_Core_State", raw_data)
